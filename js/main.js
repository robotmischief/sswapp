...
const windBtnDayOne = document.getElementById('three').querySelector('.compass');
const windBtnDayTwo = document.getElementById('two').querySelector('.compass');
const windBtnDayThree = document.getElementById('one').querySelector('.compass');
...
windBtnLastDay.addEventListener('click', function() {openModal(0);});
windBtnDayThree.addEventListener('click', function() {openModal(1);});
windBtnDayTwo.addEventListener('click', function() {openModal(2);});
windBtnDayOne.addEventListener('click', function() {openModal(3);});
...
function checkPlaceLocation(place) {
  //TODO: workaround to not exposing API keys... endpoints using express?
  const appid = '8b60c4a88bc20b12eba656211f177d81';
  const urlEarthWeather = `https://api.openweathermap.org/data/2.5/weather?q=${place}&appid=${appid}`;
  fetch(urlEarthWeather)
  .then(response => {
    if(!response.ok){
      notification('Earth Error Connection', 'There was a problem contacting the server. Using local data instead.', 'warning');
  ...
  .then(data => {
    if(data.cod === '404'){
      notification('Place Not Found', 'Cant find that place. If spelling is Ok then there is no weather stations for that location.', 'normal');
...
/*
* @description Fetchs data from InSight mission API
*/
  ...
  const apiKey = 'DEMO_KEY';//for debug
    ...
    .then(response =>{
      if(!response.ok) {
        ...
        document.querySelector('#mars-load img').style.transform = 'translateY(-50px)';
        document.querySelector('#mars-load .vert-bar').style.transform = 'translateY(-50px)';
        notification('Mars Error Connection', 'There was a problem contacting the server. Using local data instead.', 'warning');
 ...
 */
 function deployMarsUIwithData() {
  ...
  //check which sols have data
   ...
   document.getElementById('at-low-sol1').innerHTML = 
   (metric)
   ...
   //Sol 2 flipping card
    ...
    document.getElementById('at-low-sol2').innerHTML = 
    (metric)
    ...
    document.getElementById('at-low-sol3').innerHTML = 
    (metric)
    ...
    document.getElementById('at-low-sol4').innerHTML = 
    (metric)
...
function windGraph (data, solNumber) {
 ...
 const values = (new Array(16)).fill(0).map((empty, index) => (data[index]) ? data[index].ct : 0);
...
function fRotateCard (e) {
  if(actualPlanet === 'earth') return;
    const pLenght = e.path.length - pathDayPosition;
    const dayId = e.path[pLenght].id;
...
function getGenericPlanetData(planetData, metric) {
    ...
    const planetName = document.querySelector('.generic-planet-container .container-header .title h1');
    planetName.innerHTML = planetData.name.toUpperCase();
    const planetType = document.querySelector('.generic-planet-container .place h2');
    planetType.innerHTML= planetData.type;
    const planetImage = document.querySelector('.generic-planet-container .planets-header .planet img');
    ...
    yourHeight.innerHTML = `${Math.round(value)}<span class='unit'>${symbol}</span>`;
    //distance from the Sun
    const distanceValue = document.querySelector('.distance-sun .text-large');
    value = (metric) ? planetData.distance : Math.round(planetData.distance / 1.609);
...
function getEarthWeatherCheck() {
    ...
    const updateInterval = (today.getTime() - latestCheck.getTime()) / (1000 * 3600 * 24);
    if ( updateInterval > 1 ) { //time to check for new weather data
      updateEarthWeather()
    }
}
...
function updateEarthWeather() {
  ...
  //TODO: workaround to avoid exposing API key... maybe endpoint with express?
  const appid = '8b60c4a88bc20b12eba656211f177d81';
  const urlEarthWeather = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=current,minutely,hourly,alerts&units=metric&appid=${appid}`;
  fetch(urlEarthWeather)
  .then(response => {
    if(!response.ok) {
      ...
      document.querySelector('#earth-load img').style.transform = 'translateY(-50px)';
      document.querySelector('#earth-load .vert-bar').style.transform = 'translateY(-50px)';
      notification('Earth Error Connection', 'There was a problem contacting the server. Using local data instead.', 'warning');
  ...
  .catch(error => {
    ...
    document.querySelector('#earth-load .circle-dot').classList.add('error');
    document.querySelector('#earth-load .vert-bar').classList.add('error');
    console.log("earth catch error", error);
  });
}
...
function deployEarthUIwithData(){
  ...
  : `${Math.round(earthWeather.daily[0].temp["max"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('temp-today-mn').innerHTML = 
  (metric)
  ? `${Math.round(earthWeather.daily[0].temp["min"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[0].temp["min"] * 1.8 + 32)}<span>F</span>`;
  //pressure
  document.getElementById('pr-today-mx').innerHTML = `${Math.round(earthWeather.daily[0].pressure)}<span>Pa</span>`;
  //wind
  document.getElementById('wd-today-mx').innerHTML = 
  (metric)
  ? `${Math.round(earthWeather.daily[0].wind_speed)}<span>m/s</span>`
  : `${Math.round(earthWeather.daily[0].wind_speed * 2.37 + 32)}<span>mph</span>`;
  document.getElementById('wd-today-dir').innerHTML = getWindDirection(earthWeather.daily[0].wind_deg);
  document.getElementById('today-moon').src = `./images/moon_${moonPhase(today)}.svg`;
  //Three day forecast
  //day 1
  document.getElementById('day1-day').innerHTML = getStringDay(day1, false);
  document.getElementById('temp-day1-mx').innerHTML =
  (metric)
  ? `${Math.round(earthWeather.daily[1].temp["max"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[1].temp["max"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('temp-day1-mn').innerHTML = 
  (metric)
  ? `${Math.round(earthWeather.daily[1].temp["min"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[1].temp["min"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('day1-moon').src = `./images/moon_${moonPhase(day1)}.svg`;
  //day 2
  document.getElementById('day2-day').innerHTML = getStringDay(day2, false);
  document.getElementById('temp-day2-mx').innerHTML =
  (metric)
  ? `${Math.round(earthWeather.daily[2].temp["max"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[2].temp["max"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('temp-day2-mn').innerHTML = 
  (metric)
  ? `${Math.round(earthWeather.daily[2].temp["min"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[2].temp["min"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('day2-moon').src = `./images/moon_${moonPhase(day2)}.svg`;
  //day 3
  document.getElementById('day3-day').innerHTML = getStringDay(day3, false);
  document.getElementById('temp-day3-mx').innerHTML =
  (metric)
  ? `${Math.round(earthWeather.daily[3].temp["max"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[3].temp["max"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('temp-day3-mn').innerHTML = 
  (metric)
  ? `${Math.round(earthWeather.daily[3].temp["min"])}<span>C</span>`
  : `${Math.round(earthWeather.daily[3].temp["min"] * 1.8 + 32)}<span>F</span>`;
  document.getElementById('day3-moon').src = `./images/moon_${moonPhase(day3)}.svg`;
  //bottom note
...
function handlePlanetClick(e){
  ...
  if (actualPlanet === 'earth' || actualPlanet === 'mars' || actualPlanet === 'preferences') {
  ...
  if (planet === 'earth' || planet === 'mars' || planet  === 'preferences') {
    note.innerHTML = '';
      ...
      document.querySelector('.generic-planet-container').classList.remove('show');
...
*/
function closeFAB(e) {
  if (e.x > 100) { //not over the navbar
...
function loadDataSecuence() {
  ...
  setTimeout(function () {
    ...
    document.querySelector('#mercury-load img').style.transform = 'translateY(-100px)';
    document.querySelector('#mercury-load .vert-bar').style.transform = 'translateY(-100px)';
  }, 1000);
  setTimeout(function () {
    info.innerHTML = 'using stadistical data';
    document.querySelector('#mercury-load img').style.transform = 'translateY(-30px)';
    document.querySelector('#mercury-load .vert-bar').style.transform = 'translateY(-30px)';
    document.querySelector('#mercury-load .circle-dot').classList.add('warning');
  ...
  setTimeout(function () {
    ...
    info.innerHTML = 'no weather station available';
    document.querySelector('#neptune-load img').style.transform = 'translateY(-100px)';
    document.querySelector('#neptune-load .vert-bar').style.transform = 'translateY(-100px)';
    //Jupiter half up
    document.querySelector('#jupiter-load img').style.transform = 'translateY(-50px)';
    document.querySelector('#jupiter-load .vert-bar').style.transform = 'translateY(-50px)';
  }, 2600);
  //Jupiter up - Neptune down
  setTimeout(function () {
    title.innerHTML = 'Neptune';
    info.innerHTML = 'no weather station available';
    document.querySelector('#neptune-load img').style.transform = 'translateY(-50px)';
    document.querySelector('#neptune-load .vert-bar').style.transform = 'translateY(-50px)';
    document.querySelector('#neptune-load .circle-dot').classList.add('warning');
    ...
    setTimeout(function () {
      ...
      info.innerHTML = 'using stadistical data';
      document.querySelector('#jupiter-load img').style.transform = 'translateY(-100px)';
      document.querySelector('#jupiter-load .vert-bar').style.transform = 'translateY(-100px)';
    }, 3500);
    //Jupiter down
    setTimeout(function () {
      title.innerHTML = 'Jupiter';
      info.innerHTML = 'using stadistical data';
      document.querySelector('#jupiter-load img').style.transform = 'translateY(-70px)';
      document.querySelector('#jupiter-load .vert-bar').style.transform = 'translateY(-70px)';
      document.querySelector('#jupiter-load .circle-dot').classList.add('warning');
    ...
    setTimeout(function () {
      ...
      info.innerHTML = 'no weather station available';
      document.querySelector('#uranus-load img').style.transform = 'translateY(-100px)';
      document.querySelector('#uranus-load .vert-bar').style.transform = 'translateY(-100px)';
    }, 4500);
    //Venus
    setTimeout(function () {
      title.innerHTML = 'Venus';
      info.innerHTML = 'no weather station available';
      document.querySelector('#venus-load img').style.transform = 'translateY(-100px)';
      document.querySelector('#venus-load .vert-bar').style.transform = 'translateY(-100px)';
    }, 5000);
    //Venus - Uranus down - Saturn up
    setTimeout(function () {
      title.innerHTML = 'Venus';
      info.innerHTML = 'using stadistical data';
      document.querySelector('#venus-load img').style.transform = 'translateY(-60px)';
      document.querySelector('#venus-load .vert-bar').style.transform = 'translateY(-60px)';
      document.querySelector('#uranus-load img').style.transform = 'translateY(-30px)';
      document.querySelector('#uranus-load .vert-bar').style.transform = 'translateY(-30px)';
      document.querySelector('#saturn-load img').style.transform = 'translateY(-100px)';
      document.querySelector('#saturn-load .vert-bar').style.transform = 'translateY(-100px)';
    ...
    setTimeout(function () {
      ...
      document.querySelector('#uranus-load img').style.transform = 'translateY(-30px)';
      document.querySelector('#uranus-load .vert-bar').style.transform = 'translateY(-30px)';
      document.querySelector('#saturn-load img').style.transform = 'translateY(-40px)';
      document.querySelector('#saturn-load .vert-bar').style.transform = 'translateY(-40px)';
      document.querySelector('#earth-load img').style.transform = 'translateY(-100px)';
      document.querySelector('#earth-load .vert-bar').style.transform = 'translateY(-100px)';
    ...
    setTimeout(function () {
      title.innerHTML = 'Mars';
      info.innerHTML = 'contacting weather station';
      document.querySelector('#mars-load img').style.transform = 'translateY(-100px)';
      document.querySelector('#mars-load .vert-bar').style.transform = 'translateY(-100px)';
      getMarsWeatherCheck();
    }, 7500);
    //silly nerd Text
    setTimeout(function () {
